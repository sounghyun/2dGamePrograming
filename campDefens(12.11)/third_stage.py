...
def enter():
    global main_background, weather, timer, score, Life, buff
...
def exit():
    global main_background, timer, score, Life, weather
...
def create_object(frame_time, objects):
    ...
    if total_time > (random.randint(20, 30) * 0.1):
        ...
        if random.randint(0,3) : objects.append(monstertype[random.randint(0,1)]())
        else : objects.append(friendlytype[random.randint(0,1)]())
        total_time = 0.0
...
def game_end(time, score):
    if time.time_over == True:
        if score >= 6000 :
            game_framework.push_state(fourth_stage)
        else:
