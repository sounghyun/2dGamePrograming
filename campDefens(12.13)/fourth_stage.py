...
def enter():
    global main_background, timer, score, Life, buff, message
...
def create_object(frame_time, objects):
    ...
    if total_time > (random.randint(20, 30) * 0.1):
        ...
        if random.randint(0,3) : objects.append(monstertype[random.randint(0,2)]())
...
def game_end(time, score):
    if time.time_over == True:
        if score >= 8000 :
            game_framework.push_state(fifth_stage)
        else:
