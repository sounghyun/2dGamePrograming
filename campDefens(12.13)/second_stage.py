...
def create_object(frame_time, objects):
    ...
    if total_time > (random.randint(20, 30) * 0.1):
        ...
        if random.randint(0,4) : objects.append(monstertype[random.randint(0,1)]())
        else : objects.append(friendlytype[0]())
        total_time = 0.0
...
def game_end(time, score):
    if time.time_over == True:
        if score >= 4000 :
            game_framework.push_state(third_stage)
        else:
