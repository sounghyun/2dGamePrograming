...
class Bonus:
    ...
    def handle_die(self, frame_time):
        global frist_stage
        frist_stage.Life.life += monster_data['Bonus']['life']
        if random.randint(0,1):
            if frist_stage.buff.speed_buff == frist_stage.buff.speed_normal:
                if random.randint(0,1) : frist_stage.speed_up()
                else : frist_stage.speed_down()
        else:
            if frist_stage.buff.hp_buff == frist_stage.buff.hp_normal:
                if random.randint(0,1) : frist_stage.hp_up()
                else : frist_stage.hp_down()
        self.alive = False
